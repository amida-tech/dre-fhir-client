...
var _searchAll = function _searchAll(urlstring, callback) {
    request.get(urlstring, function (err, response, bundle) {
        if (err) {
            ...
            if (next) {
                _searchAll(next.url, function (err, remaining) {
                    if (err) {
                        callback(err);
                    } else {
...
exports.searchAll = function searchAll(serverInfo, resourceType, parameters, callback) {
    var urlstring = urlwrap.search(serverInfo, resourceType, parameters);
    _searchAll(urlstring, callback);
};
